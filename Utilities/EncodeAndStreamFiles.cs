...
namespace AzureMediaStreaming.Utilities
{
    class EncodeAndStreamFiles
    {
        private const string AdaptiveStreamingTransformName = "MyTransformWithAdaptiveStreamingPreset";
        private const string OutputFolderName = @"Output";

        private static ILogger<EncodeAndStreamFiles> _logger;
        private static ClientSettings _settings;

        public EncodeAndStreamFiles(IOptions<ClientSettings> settings, ILogger<EncodeAndStreamFiles> logger)
        {
            _settings = settings.Value;
            _logger = logger;
        }

        public async Task RunEncoding()
        {
            try
            {
                await RunAsync(_settings);
            }
            catch (Exception exception)
            {
                if (exception.Source?.Contains("ActiveDirectory") ?? false)
                {
                    _logger.LogError(
        ...
        /// </summary>
        /// <param name="settings">The parm is of type settingsWrapper. This class reads values from local settingsuration file.</param>
        /// <returns></returns>
        // <RunAsync>
        private async Task RunAsync(ClientSettings settings)
        {
            ...
            Transform transform = await GetOrCreateTransformAsync(client, settings.ResourceGroup, settings.AccountName,
                AdaptiveStreamingTransformName);

            // Output from the encoding Job must be written to an Asset, so let's create one
            Asset outputAsset =
                await CreateOutputAssetAsync(client, settings.ResourceGroup, settings.AccountName, outputAssetName);

            Job job = await SubmitJobAsync(client, settings.ResourceGroup, settings.AccountName,
                AdaptiveStreamingTransformName, outputAsset.Name, jobName);

            // In this demo code, we will poll for Job status
            // Polling is not a recommended best practice for production applications because of the latency it introduces.
            // Overuse of this API may trigger throttling. Developers should instead use Event Grid.
            job = await WaitForJobToFinishAsync(client, settings.ResourceGroup, settings.AccountName,
            ...
            if (job.State == JobState.Finished)
            {
                ...
                await DownloadOutputAssetAsync(client, settings.ResourceGroup, settings.AccountName, outputAssetName,
                    OutputFolderName);

                StreamingLocator locator = await CreateStreamingLocatorAsync(client, settings.ResourceGroup,
                    settings.AccountName, outputAsset.Name, locatorName);

                IList<string> urls =
                    await GetStreamingUrlsAsync(client, settings.ResourceGroup, settings.AccountName, locator.Name);
        ...
        /// supplied in local settingsuration file.
        /// </summary>
        /// <param name="settings">The parm is of type settingsWrapper. This class reads values from local settingsuration file.</param>
        /// <returns></returns>
        // <GetCredentialsAsync>
        private static async Task<ServiceClientCredentials> GetCredentialsAsync(ClientSettings settings)
        {
            ...
            ClientCredential clientCredential = new ClientCredential(settings.AadClientId, settings.AadSecret);
            return await ApplicationTokenProvider.LoginSilentAsync(settings.AadTenantId, clientCredential,
        ...
        /// supplied in local appsettings file.
        /// </summary>
        /// <param name="settings">The parm is of type settingsWrapper. This class reads values from local settingsuration file.</param>
        /// <returns></returns>
        // <CreateMediaServicesClient>
        private static async Task<IAzureMediaServicesClient> CreateMediaServicesClientAsync(ClientSettings settings)
        {
            var credentials = await GetCredentialsAsync(settings);
            ...
            string outputFolderName)
        {
            ...
            do
            {
                ...
                BlobResultSegment segment = await container.ListBlobsSegmentedAsync(null, true, BlobListingDetails.None,
                    ListBlobsSegmentMaxResult, continuationToken, null, null);
